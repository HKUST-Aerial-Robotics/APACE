...
void Controller::update(const Desired_State_t &des, const Odom_Data_t &odom,
  ...
  /*ROS_ASSERT_MSG(is_configured, "Gains for controller might not be
  initialized!"); std::string constraint_info(""); Vector3d e_p, e_v, F_des;
  double e_yaw = 0.0;
  ...
  for (size_t k = 0; k < 3; ++k) {
          if (std::fabs(u_v_i(k)) > integration_output_limits[k]) {
                  uav_utils::limit_range(u_v_i(k),
  integration_output_limits[k]); ROS_INFO("Integration saturate for axis %zu,
  value=%.3f", k, u_v_i(k));
          }
  }
  //proble is u_v_i
  ...
  if (std::fabs(F_des(0)/F_des(2)) > std::tan(toRad(50.0)))
  {
          ...
          F_des(0) = F_des(0)/std::fabs(F_des(0)) * F_des(2) *
  std::tan(toRad(30.0));
  }
  ...
  if (std::fabs(F_des(1)/F_des(2)) > std::tan(toRad(50.0)))
  {
          ...
          F_des(1) = F_des(1)/std::fabs(F_des(1)) * F_des(2) *
  std::tan(toRad(30.0));
  }
...

  {
          ...
          debug_F = Vector3d(0, 0, param.mass * param.gra) + Ka * param.mass *
  des.a; Vector3d debug_Fc = wRc.transpose()*debug_F; double debug_fx =
  debug_Fc(0); double debug_fy = debug_Fc(1); double debug_fz = debug_Fc(2);
  ...
  // 		ROS_INFO("Integration saturate for axis %zu, value=%.3f", k,
  // u_v_i(k));
  // 	}
  // }
  // Eigen::Vector3d u_v = u_v_p + u_v_i;
