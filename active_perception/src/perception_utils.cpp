...
PerceptionUtils::PerceptionUtils(ros::NodeHandle &nh) {
  ...
  nh.param("perception_utils/cam_fov_voxel_resolution", cam_fov_voxel_resolution_, -1.0);

  debug_pub_ = nh.advertise<sensor_msgs::PointCloud2>("perception_utils/debug", 1);
  ...
  // for (double x = 0; x <= max_dist_; x += cam_fov_voxel_resolution_) {
  //   // double y = 0;
  //   // double z = 0;
  //   // Vector3d p(x, y, z);
  //   // cam_fov_voxels_.push_back(p);

  //   double y_max = x * tan(left_angle_);
  //   double z_max = x * tan(top_angle_);
  //   for (double y = -y_max; y <= y_max; y += cam_fov_voxel_resolution_) {
  //     for (double z = -z_max; z <= z_max; z += cam_fov_voxel_resolution_) {
  //       cam_fov_voxels_.push_back(Vector3d(x, y, z));
  //     }
  //   }
  // }
