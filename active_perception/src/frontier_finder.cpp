...
FrontierFinder::FrontierFinder(const EDTEnvironment::Ptr &edt, ros::NodeHandle &nh) {
  this->edt_env_ = edt;
  int voxel_num = edt->map_server_->getVoxelNum();
  ...
  nh.param("frontier/min_candidate_occupied_clearance", min_candidate_occupied_clearance_, -1.0);
  ...
  nh.param("frontier/ground_height", ground_height_, 0.0);
  nh.param("frontier/ground_offset", ground_offset_, 0.0);
...
void FrontierFinder::searchFrontiers() {
  ...
  edt_env_->map_server_->getTSDF()->getUpdatedBox(update_min, update_max, false);
  // ROS_INFO("Updated box: (%.2f, %.2f, %.2f) - (%.2f, %.2f, %.2f)", update_min(0), update_min(1),
  //          update_min(2), update_max(0), update_max(1), update_max(2));
  ...
  auto resetFlag = [&](list<Frontier>::iterator &iter, list<Frontier> &frontiers) {
    ...
    for (auto cell : iter->cells_) {
      edt_env_->map_server_->getTSDF()->positionToIndex(cell, idx);
  ...
  edt_env_->map_server_->getBox(box_min, box_max);
  for (int k = 0; k < 3; ++k) {
    search_min[k] = max(search_min[k], box_min[k]);
    search_max[k] = min(search_max[k], box_max[k]);
  }
  Eigen::Vector3i min_id, max_id;
  edt_env_->map_server_->getTSDF()->positionToIndex(search_min, min_id);
  edt_env_->map_server_->getTSDF()->positionToIndex(search_max, max_id);
...
void FrontierFinder::expandFrontier(
  ...
  edt_env_->map_server_->getTSDF()->indexToPosition(first, pos);
  ...
  while (!cell_queue.empty()) {
    ...
    for (auto nbr : nbrs) {
      ...
      // clustered
      int adr = toadr(nbr);
      if (frontier_flag_[adr] == 1 || !edt_env_->map_server_->isInBox(nbr) ||
          !(knownfree(nbr) && isNeighborUnknown(nbr))) {
        continue;
      }

      edt_env_->map_server_->getTSDF()->indexToPosition(nbr, pos);
...
bool FrontierFinder::isInBoxes(const vector<pair<Vector3d, Vector3d>> &boxes,
  ...
  Vector3d pt;
  edt_env_->map_server_->getTSDF()->indexToPosition(idx, pt);
  for (auto box : boxes) {
...
void FrontierFinder::updateFrontierCostMatrix() {
  ...
  auto updateCost = [](const list<Frontier>::iterator &it1, const list<Frontier>::iterator &it2) {
    ...
    double cost_ij = ViewNode::computeCost(vui.pos_, vuj.pos_, vui.yaw_, vuj.yaw_,
                                           Vector3d(0, 0, 0), 0, path_ij);
...
void FrontierFinder::getFullCostMatrix(const Vector3d &cur_pos, const Vector3d &cur_vel,
  ...
  if (false) {
    ...
    // std::cout << "mat size: " << mat.rows() << ", " << mat.cols() <<
...
void FrontierFinder::getSwarmCostMatrix(const vector<Vector3d> &positions,
                                        ...
                                        const vector<Eigen::Vector3d> &grid_pos,
                                        Eigen::MatrixXd &mat) {
  ...
  if (!grid_pos.empty()) {
    ...
    for (int i = 0; i < ftr_num; ++i) {
      double cost = ViewNode::computeCost(next_grid, points[ftr_ids[i]], 0, 0,
                                          Eigen::Vector3d(0, 0, 0), 0, tmps);
...
void FrontierFinder::getSingleCellCostMatrix(const Vector3d &cur_pos, const Vector3d &cur_vel,
  ...
  for (int i = 0; i < ftr_ids.size(); i++) {
    ...
    vector<Vector3d> path;
    mat(0, i + 1) =
        ViewNode::computeCost(cur_pos, vj.pos_, cur_yaw[0], vj.yaw_, cur_vel, 0.0, path);
  ...
  for (int i = 0; i < ftr_ids.size(); i++) {
    ...
    mat(i + 1, dimension - 1) =
        ViewNode::computeCost(vj.pos_, next_grid_pos, 0.0, 0.0, Eigen::Vector3d::Zero(), 0.0, path);
    mat(dimension - 1, i + 1) = 1000;
  }
}

void FrontierFinder::findViewpoints(const Vector3d &sample, const Vector3d &ftr_avg,
                                    vector<Viewpoint> &vps) {
  if (!edt_env_->map_server_->isInBox(sample) ||
      edt_env_->map_server_->getOccupancy(sample) == voxel_mapping::OccupancyType::OCCUPIED ||
  ...
  for (double phi_h = -M_PI_2; phi_h <= M_PI_2 + 1e-3; phi_h += M_PI / 18) {
    ...
    for (double phi_v = -vertical_angle_; phi_v <= vertical_angle_; phi_v += vertical_angle_ / 3) {
      ...
      while (raycaster_->nextId(idx)) {
        ...
        if (edt_env_->map_server_->getOccupancy(sample) == voxel_mapping::OccupancyType::OCCUPIED ||
            !edt_env_->map_server_->isInBox(idx))
          break;
        // Count number of unknown cells
        if (edt_env_->map_server_->getOccupancy(idx) == voxel_mapping::OccupancyType::UNKNOWN)
          ++gain;
      }
    }
  ...
  for (int i = 0; i < 6; ++i) // [-90,-10]-> [10,90], delta_yaw = 20, 6 groups
  {
    ...
    for (int j = 2 * i; j < 2 * i + 9; ++j) // 80 degree hFOV, 9 slices
      gain += slice_gains[j];
    yaw_gains.push_back(std::make_pair(yaw, gain));
  }
...
void FrontierFinder::sampleViewpoints(Frontier &frontier) {
  ...
  std::vector<fast_planner::Viewpoint> viewpoints;
  ...
  for (double rc = candidate_rmin_, dr = (candidate_rmax_ - candidate_rmin_) / candidate_rnum_;
    ...
    for (double phi = -M_PI; phi < M_PI; phi += candidate_dphi) {
      ...
      if (!edt_env_->map_server_->isInBox(sample_pos) ||
          edt_env_->map_server_->getOccupancy(sample_pos) ==
              voxel_mapping::OccupancyType::OCCUPIED ||
          edt_env_->map_server_->getOccupancy(sample_pos) ==
              voxel_mapping::OccupancyType::UNKNOWN ||
          isNearUnknown(sample_pos) || isNearOccupied(sample_pos))
        continue;
  ...
  for (int i = 0; i < viewpoints.size(); ++i) {
    ...
    if (!edt_env_->map_server_->getTSDF()->isInMap(vp.pos_)) {
      continue;
    }
    ...
    for (const Position &p : voxels_cam) {
      ...
      if (!edt_env_->map_server_->getTSDF()->isInMap(p_w)) {
        // Find nearest point in box
        p_w = edt_env_->map_server_->getTSDF()->closestPointInMap(p_w, vp.pos_);
      }
      ...
      bool blocked = false;
      while (raycaster_->nextId(idx)) {
        if (edt_env_->map_server_->getOccupancyGrid()->isInMap(idx) == false) {
          // cout << "vp.pos_: " << vp.pos_.transpose() << endl;
          // cout << "p_w: " << p_w.transpose() << endl;
          // cout << "idx pos: " <<
          // edt_env_->map_server_->getTSDF()->indexToPosition(idx).transpose()
          //      << endl;
          // //  raycaster start and end
          // cout << "caster start pos: " << ViewNode::caster_->getStartPos().transpose() << endl;
          // cout << "caster start idx: " << ViewNode::caster_->getStartIndex().transpose() << endl;
          // cout << "caster end pos: " << ViewNode::caster_->getEndPos().transpose() << endl;
          // cout << "caster end idx: " << ViewNode::caster_->getEndIndex().transpose() << endl;
          // CHECK(false) << "idx: " << idx.transpose() << endl;
          break;
        }

        if (edt_env_->map_server_->getOccupancyGrid()->getVoxel(idx).value ==
            voxel_mapping::OccupancyType::OCCUPIED) {
          blocked = true;
          break;
        }
        ...
        if (edt_env_->map_server_->getOccupancyGrid()->getVoxel(idx).value ==
            voxel_mapping::OccupancyType::UNKNOWN) {
          voxels_world_unknown.push_back(edt_env_->map_server_->getTSDF()->indexToPosition(idx));
        }
      }
      if (blocked)
        continue;
...
bool FrontierFinder::isFrontierCovered() {
  Vector3d update_min, update_max;
  edt_env_->map_server_->getTSDF()->getUpdatedBox(update_min, update_max);
  ...
  for (int x = -vox_num; x <= vox_num; ++x)
    for (int y = -vox_num; y <= vox_num; ++y)
      for (int z = -1; z <= 1; ++z) {
        ...
        if (edt_env_->map_server_->getOccupancy(vox) == voxel_mapping::OccupancyType::UNKNOWN)
          return true;
      }
  return false;
}
  ...
  for (int x = -vox_num; x <= vox_num; ++x)
    for (int y = -vox_num; y <= vox_num; ++y)
      for (int z = -vox_num; z <= vox_num; ++z) {
        Eigen::Vector3d vox;
        vox << pos[0] + x * resolution_, pos[1] + y * resolution_, pos[2] + z * resolution_;
        if (edt_env_->map_server_->getOccupancy(vox) == voxel_mapping::OccupancyType::OCCUPIED)
          return true;
      }
  return false;
}
...
int FrontierFinder::countVisibleCells(const Eigen::Vector3d &pos, const double &yaw,
  ...
  for (auto cell : cluster) {
    ...
    bool visib = true;
    while (raycaster_->nextId(idx)) {
      if (edt_env_->map_server_->getOccupancy(idx) == voxel_mapping::OccupancyType::OCCUPIED ||
          edt_env_->map_server_->getOccupancy(idx) == voxel_mapping::OccupancyType::UNKNOWN) {
        visib = false;
        break;
      }
    }
...
void FrontierFinder::downsample(const vector<Eigen::Vector3d> &cluster_in,
  ...
  const double leaf_size = edt_env_->map_server_->getResolution() * down_sample_;
...
inline bool FrontierFinder::isNeighborUnknown(const Eigen::Vector3i &voxel) {
  ...
  for (auto nbr : nbrs) {
    if (edt_env_->map_server_->getOccupancy(nbr) == voxel_mapping::OccupancyType::UNKNOWN)
      return true;
  }
  return false;
}
...
inline int FrontierFinder::toadr(const Eigen::Vector3i &idx) {
  return edt_env_->map_server_->getTSDF()->indexToAddress(idx);
}

inline bool FrontierFinder::knownfree(const Eigen::Vector3i &idx) {
  return edt_env_->map_server_->getOccupancy(idx) == voxel_mapping::OccupancyType::FREE;
}

inline bool FrontierFinder::inmap(const Eigen::Vector3i &idx) {
  return edt_env_->map_server_->getTSDF()->isInMap(idx);
}
